...
// Normalise a path string/array for comparison (drop whitespace, upper-case).
function normPath(p) {
    return (Array.isArray(p) ? p.join(';') : (p ?? '')).replace(/\s+/g, '').toUpperCase()
}

// Invert the forward Silva algorithm: find the (group, subtype) whose loop notation
// reproduces `targetPath` for the structure's tetrad count. Returns nulls if no
// classification data, no path, or no subtype reproduces the path exactly.
function classifyPath(targetPath, structure, silvaData) {
    const target = normPath(targetPath)
    if (!target || !silvaData?.length) return { group: null, subtype: null }
    const tetrads = Math.max(1, countTetrads(structure) || 1)
    for (const g of silvaData) {
        for (const s of (g.subtypes ?? [])) {
            if (!s.loop) continue
            try {
                if (normPath(buildPath(s.loop, tetrads)) === target)
                    return { group: g.code, subtype: s.code }
            } catch { /* unparseable loop notation — skip */ }
        }
    }
...
export default function SequenceForm({ onRun, runState, prefill }) {
    ...
    const appliedPrefillToken = useRef(null)
    ...
    // the structure's tetrad count, find the (group, subtype) whose loop notation
    // reproduces the generated path via buildPath(). This is the PRIMARY, universal
    // mechanism: buildPath round-trips exactly and the 26 subtypes map to distinct
    // paths, so the match is unique for any canonical threading (verified). The
    // example hint (prefill.silvaGroup) is only a fallback when the path isn't a
    // canonical threading. silvaData may load after the prefill arrives (first Build
    // mount), so this effect re-runs on silvaData and refines the classification.
    useEffect(() => {
        if (!prefill?._token) return

        const pathStr = Array.isArray(prefill.path) ? prefill.path.join(';') : (prefill.path ?? '')
        // 1) actual generated topology (authoritative when silvaData is loaded)
        let { group: matchedGroup, subtype: matchedSub } = classifyPath(pathStr, prefill.structure, silvaData)
        // 2) fall back to the example hint only if the path didn't classify
        if (!matchedGroup && prefill.silvaGroup) {
            matchedGroup = prefill.silvaGroup
            matchedSub   = prefill.silvaSubtype ?? null
        }

        if (prefill._token !== appliedPrefillToken.current) {
            appliedPrefillToken.current = prefill._token

            const seq    = prefill.sequence ?? ''
            const pucker = prefill.Shugar?.trim()
                ? prefill.Shugar.trim()
                : seq.split('').map(ch => /[A-Z]/.test(ch) ? 'N' : 'S').join('')

            const snapshot = {
                nameVal:    prefill.name ?? '',
                seqVal:     seq,
                structVal:  prefill.structure ?? '',
                pathVal:    pathStr,
                chiVal:     prefill.chi ?? '',
                orientVal:  prefill.orient || 'A+;B-',
                twist:      String(prefill.twist ?? '29'),
                rise:       formatRise(prefill.rise ?? '3.4'),
                pucker,
                silvaGroup: matchedGroup, subtype: matchedSub, advOpen: true,
            }
            // Seed both modes so the mode toggle restores this data instead of resetting.
            savedStates.current.canonical    = { ...snapshot }
            savedStates.current.noncanonical = { ...snapshot }
            ...
            setChiVal(snapshot.chiVal)
            setOrientVal(snapshot.orientVal)
            setTwist(snapshot.twist)
            setRise(snapshot.rise)
            setPathVal(snapshot.pathVal)
            setPucker(snapshot.pucker)
